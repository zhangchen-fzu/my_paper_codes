...
biaoqian=pd.read_csv(r'D:\my paper aa\第四次整体结果整合\all.csv',usecols=[2]).values.tolist()
biaoqian=list(itertools.chain.from_iterable(biaoqian))
all_num=sum(biaoqian)  #计算召回率的
    ...
    for i in range(len(group_data)):
        if group_data['y_hat'][i] in max_value:
            ...
            if group_data['biaoqian'][i]==1:
                num_precision+=1
pre=num_precision/choice_num
recall=num_precision/all_num
